...
def crop(img, center, scale, output_size, rot=0):
    ...
    if sf < 2:
        ...
        # 使用 Pillow 替代 scipy.misc.imresize
        pil_img = Image.fromarray(img.astype(np.uint8))
        pil_img = pil_img.resize((new_wd, new_ht), Image.BILINEAR)
    ...
    if not rot == 0:
        # Remove padding
        if rot != 0:
            pil_img = Image.fromarray(new_img.astype(np.uint8))
            pil_img = pil_img.rotate(rot, resample=Image.BILINEAR, expand=False)
            new_img = np.array(pil_img)
        new_img = new_img[pad:-pad, pad:-pad]
    pil_img = Image.fromarray(new_img.astype(np.uint8))
    pil_img = pil_img.resize(output_size, Image.BILINEAR)  # 双线性插值
    new_img = np.array(pil_img)
    return new_img
